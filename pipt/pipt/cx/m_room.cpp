...
void MRoom::initw(){
	...
	GLint last_texg;
	glGetIntegerv(GL_TEXTURE_BINDING_2D, (GLint*)&last_texg);
...
void MRoom::drawfloor(GLfloat* eye,GLfloat* light){
	...
	//Set vertex arrays for wall
	glVertexPointer(3, GL_FLOAT, sizeof(FLOOR_VERTEX), &myfloor.vertices[0].position);
	glEnableClientState(GL_VERTEX_ARRAY);

	glNormalPointer(GL_FLOAT, sizeof(FLOOR_VERTEX), &myfloor.vertices[0].normal);
	glEnableClientState(GL_NORMAL_ARRAY);

	glTexCoordPointer(2, GL_FLOAT, sizeof(FLOOR_VERTEX), &myfloor.vertices[0].s);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
